...
module.exports = (handler) => {
  ...
  parser.on('closetag', (name) => handler.onCloseTag(name))
...
function _initParser(parser) {
  parser.write = function (data) {
    ...
    if (
      ...
      if (!this._decoder) {
        const SD = require('node:string_decoder').StringDecoder
        this._decoder = new SD('utf8')
      }
      decodedData = this._decoder.write(data)
    }

    this._parser.write(decodedData.toString())
    return true
  }
